...
def _metadata(text, language, sources=None, elapsed=0.0):
    return {
        "text": text,
...
def _is_storage_chunk(chunk):
    ...
    # Only trust the precomputed topic tags / source naming, not a loose scan
    # of the raw text. A maize-planting chunk that mentions "harvest" once in
    # passing is NOT a storage chunk, and treating it as one was the main
    # cause of valid maize-production sources being filtered out entirely
    # (producing false "no information" answers).
...
def _filter_chunks(question: str, chunks):
    ...
    for chunk in chunks:
        ...
        if explicit_storage and "production" not in intents:
            # For storage questions, keep crop/general storage chunks but drop
            # planting-only chunks that do not mention storage at all.
    ...
    if not selected and chunks:
        # The topic/crop/storage filters above are precision filters, not a
        # relevance gate. If they ended up rejecting everything even though
        # the retriever found chunks comfortably above the score cutoff,
        # that is a filter bug, not genuine "no information" - fall back to
        # the best-scoring raw chunks instead of telling the farmer we know
        # nothing.
...
def _instant_response(question: str, language: str):
    ...
    if normalized in THANKS_WORDS or (
        ...
        if language == "hausa" or "gode" in normalized:
            return _metadata("Babu komai. Za ka iya tambaya game da noma, taki, kwari, kiwo, ko ajiya.", language)
        return _metadata("You're welcome. Ask me anytime about crops, livestock, fertilizer, pests, or storage.", language)
    return None
