...
AgriSec Local Assistant is an offline agricultural advisory tool for the ADTC Agriculture domain. It runs locally through Ollama and llama.cpp, retrieves from a local agriculture corpus, and answers with visible source files so advice can be inspected.

## 2. Identified Constraints

| Constraint | Description |
|---|---|
| Compute | CPU-oriented inference on the ADTC Standard Laptop profile (4 vCPU, 8 GB RAM) |
...
|---|---|---|---|
| Base model | Llama 3.2 1B/3B, Gemma 2B, Qwen 1.5B/3B | Qwen2.5:1.5B Instruct (Q4_K_M GGUF) | Small, high reasoning fidelity, easy local deployment |
| Retrieval | FAISS, Chroma, no RAG | FAISS CPU | Lightweight, local, no external server process |
| Embeddings | English MiniLM, multilingual MiniLM | `paraphrase-multilingual-MiniLM-L12-v2` | Superior English/Hausa cross-lingual retrieval |
| Interface | CLI only, desktop shell, browser UI | Lightweight localhost web UI plus CLI | Good judging UX without heavy RAM cost |
| Backend | FastAPI, Flask, Python stdlib server | Python stdlib `http.server` | Zero external framework dependencies; pure offline setup |
...
- **Qwen2.5 1.5B Instruct** - compact footprint suitable for commodity laptops while excelling at RAG tasks.
- **FAISS CPU** - fast, low-overhead local vector search over agriculture documents.
